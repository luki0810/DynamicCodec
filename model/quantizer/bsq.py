...
class BinarySphericalQuantizer(AbsQuantizer):
    def __init__(
        self,
        latent_dim: int = 128,
        codebook_dim: int = 14,
        ...
        assert (
            self.codebook_dim % group_size == 0
        ), "codebook_dim must be divisible by group_size"
        self.num_groups = self.codebook_dim // group_size
        ...
        self.register_buffer("basis", 2 ** torch.arange(codebook_dim - 1, -1, -1))
        self.register_buffer("group_basis", 2 ** torch.arange(group_size - 1, -1, -1))

        self.num_dimensions = 2**codebook_dim
        self.bits_per_index = codebook_dim
    ...
    def quantize(self, z):
        assert (
            z.shape[-1] == self.codebook_dim
        ), f"Expected {self.codebook_dim} dimensions, got {z.shape[-1]}"
    ...
    def forward(self, z):
        # 1) BDT -> BCT (投影到码空间)
        z_code_cf = self.in_proj(z)                # (B, D, T) -> (B, C, T)
        z_code = rearrange(z_code_cf, "b c t -> b t c")   # (B, T, C)

        # 2) 量化（最后一维是 C）
        zq_code = self.quantize(z_code)

        # 3) 索引 (B, T) / 分组索引 (B, T, G)
        indices = self.codes_to_indexes(zq_code.detach())
        group_indices = self.codes_to_group_indexes(zq_code.detach())
        used_codes = torch.unique(indices, return_counts=False) if not self.training else None

        # 4) 熵正则（soft 或 hard），注意这些函数都默认最后一维是 C
        q_scale = 1.0 / (self.codebook_dim ** 0.5) if self.l2_norm else 1.0
        if self.soft_entropy:
            persample_entropy, cb_entropy, avg_prob = self.soft_entropy_loss(z_code)
            entropy_penalty = self.gamma0 * persample_entropy - self.gamma * cb_entropy
        else:
            zb_by_sample = (((zq_code + 1) / 2).reshape(z_code.shape[0], -1, z_code.shape[-1]).to(torch.float32))
            persample_entropy = self.get_hard_per_sample_entropy(zb_by_sample)
            cb_entropy = codebook_entropy(zq_code, self.basis, self.codebook_dim)
            entropy_penalty = self.gamma0 * persample_entropy - self.gamma * cb_entropy
            avg_prob = None

        # 5) 缩放 + 承诺项（在 code 空间比较）
        zq_code = zq_code * q_scale
        commit_loss = torch.mean(((zq_code.detach() - z_code) ** 2).sum(dim=-1))  # 标量

        # 6) 回到 RVQ 期望的输出空间：BCT -> 通过 out_proj -> BDT
        zq_code_cf = rearrange(zq_code, "b t c -> b c t")   # (B, C, T)
        z_q = self.out_proj(zq_code_cf)                     # (B, D, T)

        # 7) 按 RVQ 风格返回
        codes = indices                    # (B, T)
        latents = z.detach()               # (B, D, T)
        loss_dict = {
            "commitment_loss": torch.Tensor(commit_loss * self.beta).unsqueeze(0),
            "entropy_penalty": torch.Tensor(self.zeta * entropy_penalty / self.inv_temperature).unsqueeze(0),
        }
        other = {
            "H": cb_entropy,
    ...
    def soft_entropy_loss(self, z):
        ...
        group_code_book = self.group_codebook / (
            self.codebook_dim**0.5 if self.l2_norm else 1
        )
    ...
    def codes_to_indexes(self, zhat):
        ...
        ), f"Expected {self.codebook_dim} dimensions, got {zhat.shape[-1]}"
